...
public class EuclideanCluster {
	...
	// 构造函数
	public EuclideanCluster(int k, double threshold, int repeat,
	...
	public Map<Integer, Double> getMergeCurveOfMap(Map<Integer, Double> map1,
			...
			throws TimeSeriesNotEquilongException {
		Map<Integer, Double> result = EuclideanCurveMerge.getMapCurveFitting(
				map1, map2, weight1, weight2);
		return result;
	}
	...
	public void classifyData2() throws DataNullException,
		...
		if (this.data == null) {
			...
			while (iter.hasNext()) {
				...
				double tempPro = EuclideanDisstance.getDistance(tempTS,
						centerMap.get(0));
				int clusterNum = 0;
				for (int i = 1; i < k; i++) {
					double temp = EuclideanDisstance.getDistance(tempTS,
							centerMap.get(i));
					if (temp < tempPro) {
						clusterNum = i;
						tempPro = temp;
	...
	public double getOptimizingValue() throws DataNullException,
		...
		if (this.data == null) {
			...
			while (iter.hasNext()) {
				TimeSeries tempTS = iter.next();
				result += EuclideanDisstance.getDistance(tempTS,
